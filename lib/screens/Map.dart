            ...
            child: Column(
              children: [
                ElevatedButton(
                    ...
                    onPressed: () async {
                      List<Location> start1 = await locationFromAddress(
                          "Sion West, Sion, Mumbai, Maharashtra");
                      List<Location> end1 = await locationFromAddress(
                          "Kurla West, Kurla, Mumbai, Maharashtra");
                      ...
                      setState(() {
                        routepoints1 = [];
                        var route1 = jsonDecode(response1.body)['routes'][0]
                        ...
                        for (int i = 0; i < route1.length; i++) {
                          ...
                          routepoints1.add(LatLng(
                              double.parse(lat1[1]), double.parse(long1[0])));
                        }
                      });

                      // Route 2
                      List<Location> start2 = await locationFromAddress(
                          "Kurla West, Kurla, Mumbai, Maharashtra");
                      List<Location> end2 = await locationFromAddress(
                          "Ghatkopar West, Ghatkopar, Mumbai, Maharashtra");
                      ...
                      var url2 = Uri.parse(
                          'http://router.project-osrm.org/route/v1/driving/$v2_2,$v1_2;$v4_2,$v3_2?steps=true&annotations=true&geometries=geojson&overview=full');
                      var response2 = await http.get(url2);
                      print(response2.body);

                      setState(() {
                        routepoints2 = [];
                        var route2 = jsonDecode(response2.body)['routes'][0]
                            ['geometry']['coordinates'];
                        for (int i = 0; i < route2.length; i++) {
                          ...
                          routepoints2.add(LatLng(
                              double.parse(lat2[1]), double.parse(long2[0])));
                        }
                      });

                      // Route 3
                      List<Location> start3 = await locationFromAddress(
                          "Ghatkopar West, Ghatkopar, Mumbai, Maharashtra");
                      List<Location> end3 = await locationFromAddress(
                          "Vikhroli West, Vikhroli, Mumbai, Maharashtra");
                      ...
                      var url3 = Uri.parse(
                          'http://router.project-osrm.org/route/v1/driving/$v2_3,$v1_3;$v4_3,$v3_3?steps=true&annotations=true&geometries=geojson&overview=full');
                      var response3 = await http.get(url3);
                      print(response3.body);

                      setState(() {
                        routepoints3 = [];
                        var route3 = jsonDecode(response3.body)['routes'][0]
                            ['geometry']['coordinates'];
                        for (int i = 0; i < route3.length; i++) {
                          ...
                          routepoints3.add(LatLng(
                              double.parse(lat3[1]), double.parse(long3[0])));
                        }
                      });
